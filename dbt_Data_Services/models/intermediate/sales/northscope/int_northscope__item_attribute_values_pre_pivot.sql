with item_attribute_begin as (
    select
        ...
        company.companyname,
        itm.itemsk as item_sk,
        case
            when atr.attribute = 'Product Size'
                then 'Size'
            else atr.attribute
        end as attribute_description,
        trim(itm.itemid) as item_id,
        trim(itm.itemid::string)
        || ' - '
        || trim(itm.itemdescription::string) as item_display_description,
    ...
    from {{ ref('stg_northscope__erpx_im_item') }} as itm

    inner join
        {{ ref('stg_northscope__erpx_mf_data_entity_company') }} as company
        on itm.dataentitycompanysk = company.dataentitycompanysk

    left outer join {{ ref('stg_northscope__erpx_im_item_class') }} as class
        on itm.itemclasssk = class.itemclasssk

    left outer join {{ ref('stg_northscope__erpx_im_item_type') }} as typ
        on itm.itemtypesk = typ.itemtypesk

    left outer join {{ ref('stg_northscope__erpx_im_uom_schedule') }} as sch
        on itm.uomschedulesk = sch.uomschedulesk

    left outer join {{ ref('stg_northscope__erpx_im_item_attributes') }} as iia
        on itm.itemsk = iia.itemsk

    left outer join {{ ref('stg_northscope__erpx_mf_attribute') }} as atr
        on iia.attributesk = atr.attributesk

    left outer join {{ ref('stg_northscope__erpx_mf_attribute_class') }} as mfac
        on itm.attributeclasssk = mfac.attributeclasssk

    left outer join {{ ref('stg_northscope__erpx_mf_attribute_value') }} as mfav
        on
            to_char(mfav.attributevaluesk) = iia.attributevalue
            and iia.attributesk = mfav.attributesk

    left outer join {{ ref('stg_northscope__erpx_im_uom') }} as uom1
        on itm.defaultsounituomsk = uom1.uomsk

    left outer join {{ ref('stg_northscope__erpx_im_uom') }} as uom2
        on itm.defaultsoweightuomsk = uom2.uomsk

    left outer join {{ ref('stg_northscope__erpx_im_uom') }} as uom3
