with main as (
    ...
    where form_name = 'NNI Outbound - Dispatched Feed'
),
...
nested_answers as (
    select
        ...
        case when na.value:controlType::string = 'BarcodeScanner' then n_val.value:data::string else n_val.value::string end as value
    from
        main,
...
outbound_header as (
    select
        ...
        max(case when question = 'Destination Area' then value end) as destination_area,
        max(case when question = 'Destination Site' then value end) as destination_site,
...
fact_outbound as (
    select
        ...
        hdr.warehouse,
        hdr.dispatch_date,
        hdr.destination_area,
        hdr.destination_site,
        hdr.site_manager,
        hdr.phone_number,
        'Trailer' as location,
        ...
        line.qr_code_data,
        line.bag_size
    from outbound_line as line

    inner join outbound_header as hdr
        on line.executionid = hdr.executionid
)

select * from fact_outbound
