with main as (
    ...
    where form_name = 'Inbound - Feed Warehouse NNI + QR'
),
...
return_header as (
    select
        ...
        form_name,
        login_name,
        max(case when question = 'Order #' then value end) as nni_order_no,
        max(case when question = 'Shipping Date' then value end) as shipping_date,
        max(case when question = 'Reception Date' then value end) as reception_date,
...
return_line as (
    select
        ...
        max(case when question = 'QRCode' then value end) as qr_code_data,
        max(case when question = 'BagSize' then value end) as bag_size
    from nested_answers
    group by all
),
