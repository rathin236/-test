...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_avc__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_avc__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_avc__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_avc__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_avc__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_avc__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_avc__gl00100') }}
),

avc_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'AVC') as company_name,
        case
...
select * from avc_ap_transactions
