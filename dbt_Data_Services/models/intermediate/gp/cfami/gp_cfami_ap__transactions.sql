...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_cfami__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_cfami__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_cfami__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_cfami__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_cfami__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_cfami__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_cfami__gl00100') }}
),

cfami_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'CFAMI') as company_name,
        case
...
select * from cfami_ap_transactions
