...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_causa__mc40000') }}
),

rm20101 as (
    select * from {{ ref('stg_gp_causa__rm20101') }}
),

rm30101 as (
    select * from {{ ref('stg_gp_causa__rm30101') }}
),

rm10101 as (
    select * from {{ ref('stg_gp_causa__rm10101') }}
),

rm30301 as (
    select * from {{ ref('stg_gp_causa__rm30301') }}
),

rm00101 as (
    select * from {{ ref('stg_gp_causa__rm00101') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_causa__gl00100') }}
),

causa_ar_transactions as (
...
where ltrim(rtrim(interid)) = 'CAUSA') as company_name,
        case
...
select * from causa_ar_transactions
