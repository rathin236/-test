...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_cos__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_cos__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_cos__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_cos__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_cos__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_cos__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_cos__gl00100') }}
),

cos_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'COS') as company_name,
        case
...
select * from cos_ap_transactions
