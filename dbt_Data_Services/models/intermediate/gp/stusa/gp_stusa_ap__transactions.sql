...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_stusa__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_stusa__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_stusa__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_stusa__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_stusa__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_stusa__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_stusa__gl00100') }}
),

stusa_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'STUSA') as company_name,
        case
...
select * from stusa_ap_transactions
