...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_gdvii__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_gdvii__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_gdvii__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_gdvii__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_gdvii__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_gdvii__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_gdvii__gl00100') }}
),

gdvii_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'GDVII') as company_name,
        case
...
select * from gdvii_ap_transactions
