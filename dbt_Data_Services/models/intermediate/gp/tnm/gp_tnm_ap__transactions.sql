...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_tnm__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_tnm__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_tnm__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_tnm__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_tnm__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_tnm__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_tnm__gl00100') }}
),

tnm_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'TNM') as company_name,
        case
...
select * from tnm_ap_transactions
