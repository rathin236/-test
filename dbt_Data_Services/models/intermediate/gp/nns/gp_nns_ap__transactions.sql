...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_nns__mc40000') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_nns__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_nns__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_nns__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_nns__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_nns__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_nns__gl00100') }}
),

nns_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'NNS') as company_name,
        case
...
select * from nns_ap_transactions
