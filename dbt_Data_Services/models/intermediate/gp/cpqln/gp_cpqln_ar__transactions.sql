...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_cpqln__mc40000') }}
),

rm20101 as (
    select * from {{ ref('stg_gp_cpqln__rm20101') }}
),

rm30101 as (
    select * from {{ ref('stg_gp_cpqln__rm30101') }}
),

rm10101 as (
    select * from {{ ref('stg_gp_cpqln__rm10101') }}
),

rm30301 as (
    select * from {{ ref('stg_gp_cpqln__rm30301') }}
),

rm00101 as (
    select * from {{ ref('stg_gp_cpqln__rm00101') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_cpqln__gl00100') }}
),
...
where ltrim(rtrim(interid)) = 'CPQLN') as company_name,
        case
