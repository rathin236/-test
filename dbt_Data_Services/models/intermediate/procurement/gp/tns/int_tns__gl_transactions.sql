...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_tns__mc40000') }}
),

mc020103 as (
    select * from {{ ref('stg_gp_tns__mc020103') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_tns__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_tns__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_tns__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_tns__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_tns__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_tns__gl00100') }}
),
...
where ltrim(rtrim(interid)) = 'TNS') as company_name,
        case
...
select * from tns_ap_transactions
