...
mc40000 as (
    select ltrim(rtrim(funlcurr)) from {{ ref('stg_gp_kcs__mc40000') }}
),

mc020103 as (
    select * from {{ ref('stg_gp_kcs__mc020103') }}
),

pm20000 as (
    select * from {{ ref('stg_gp_kcs__pm20000') }}
),

pm30200 as (
    select * from {{ ref('stg_gp_kcs__pm30200') }}
),

pm10100 as (
    select * from {{ ref('stg_gp_kcs__pm10100') }}
),

pm30600 as (
    select * from {{ ref('stg_gp_kcs__pm30600') }}
),

pm00200 as (
    select * from {{ ref('stg_gp_kcs__pm00200') }}
),

gl00100 as (
    select * from {{ ref('stg_gp_kcs__gl00100') }}
),
...
-- where current_transaction_amount
),

kcs_ap_transactions as (
...
where ltrim(rtrim(interid)) = 'KCS') as company_name,
        case
    ...
    -- ,     {{ ref('stg_gp_kcs__sy00500') }}.aprvluserid AS approver,
    --      {{ ref('stg_gp_kcs__sy00500') }}.userid AS approval_user
    from
        all_transactions
-- LEFT OUTER JOIN  {{ ref('stg_gp_kcs__sy00500') }} ON trim(bachnumb) =  trim({{ ref('stg_gp_kcs__sy00500') }}.bachnumb) 
)

select * from kcs_ap_transactions
-- where approval_user <> null
