with ap_invoices as (
    ...
    select * from {{ ref('stg_sage_100_tor__ap_invoicehistoryheader') }}

),

ap_payments as (

    select * from {{ ref('int_sage_100_tor__ap_payments') }}

),

ar_invoices as (

    select * from {{ ref('stg_sage_100_tor__ar_invoicehistoryheader') }}

),

account_master as (

    select * from {{ ref('int_sage_100_tor__account_master') }}

),

gl_details as (

    select * from {{ ref('stg_sage_100_tor__gl_detailposting') }}
