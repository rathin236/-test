with ap_invoices as (
    ...
    select * from {{ ref('stg_sage_100_del__ap_invoicehistoryheader') }}

),

ap_payments as (

    select * from {{ ref('int_sage_100_del__ap_payments') }}

),

ar_invoices as (

    select * from {{ ref('stg_sage_100_del__ar_invoicehistoryheader') }}

),

account_master as (

    select * from {{ ref('int_sage_100_del__account_master') }}

),

gl_details as (

    select * from {{ ref('stg_sage_100_del__gl_detailposting') }}
