with ap_invoices as (
    ...
    select * from {{ ref('stg_sage_100_sea__ap_invoicehistoryheader') }}

),

ap_payments as (

    select * from {{ ref('int_sage_100_sea__ap_payments') }}

),

ar_invoices as (

    select * from {{ ref('stg_sage_100_sea__ar_invoicehistoryheader') }}

),

account_master as (

    select * from {{ ref('int_sage_100_sea__account_master') }}

),

gl_details as (

    select * from {{ ref('stg_sage_100_sea__gl_detailposting') }}
