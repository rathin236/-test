with coolearth_data as (
    select * from {{ ref('int_edi_856__coolearth_cw_and_pallet_quantities') }}
),
...
ce_group as (
    ...
    left join {{ ref('stg_northscope__erpx_im_uom_schedule_conversion_value') }} as cv1
    ...
    inner join {{ ref('stg_northscope__erpx_im_uom') }} as uom
        on soi.unitsuomsk = uom.uomsk
            and soi.dataentitycompanysk = uom.dataentitycompanysk
    
    inner join {{ ref('stg_northscope__erpx_im_uom') }} as uomw
    ...
    left join {{ ref('stg_northscope__erpx_ar_customer_favorite') }} as fav
        on fav.itemsk = soi.itemsk
            and fav.customerentitysk = soh.shipaddresssk
            and fav.customerfavoritetypesk = 2 -- Address Join

    left join {{ ref('stg_northscope__erpx_ar_customer_favorite') }} as fav2
        on fav.itemsk = soi.itemsk
            and fav.customerentitysk = soh.shipaddresssk
            and fav.customerfavoritetypesk = 1 -- Customer Join

    left join {{ ref('stg_northscope__erpx_so_order_item') }} as oi2
        on oi2.orderitemsk = soi.originorderitemsk
            and soi.originitemtypeen = 3

    left join {{ ref('stg_northscope__erpx_ar_customer_substitute_item') }} as sub1
        on sub1.customerfavoritesk = fav.customerfavoritesk
            and sub1.substituteitemsk = oi2.itemsk

    left join {{ ref('stg_northscope__erpx_ar_customer_substitute_item') }} as sub2
