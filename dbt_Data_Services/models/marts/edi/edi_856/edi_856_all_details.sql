with all_details as (
    ...
    from {{ ref('edi_856_sender_group') }} as seg

    inner join {{ ref('edi_856_header_group') }} as heg
        on seg.order_id = heg.order_id

    inner join {{ ref('edi_856_routing_group') }} as rog
        on seg.order_id = rog.order_id

    inner join {{ ref('edi_856_equipment_group') }} as eqg
        on seg.order_id = eqg.order_id

    inner join {{ ref('edi_856_address_group') }} as adg
        on seg.order_id = adg.order_id

    inner join {{ ref('edi_856_order_group') }} as org
        on seg.order_id = org.order_id

    inner join {{ ref('edi_856_pallet_group_sscc') }} as pag
        on seg.order_id = pag.order_id

    inner join {{ ref('edi_856_item_group') }} as itg
