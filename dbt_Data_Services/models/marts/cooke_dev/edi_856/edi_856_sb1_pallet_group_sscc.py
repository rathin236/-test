def calculate_checksum_digit_sb1(sscc: str) -> str:
...
def model(dbt, session):
    dbt.config(materialized="table", packages=["pandas", "numpy"])
    df = dbt.ref("int_edi_856_sb1__gfs_pallet_group")
    df = df.to_pandas()

    df = df.apply(lambda x: x.str.strip() if x.dtype == "object" else x)

    df["SSCC_CHECKSUM"] = df["ID"].apply(calculate_checksum_digit_sb1)
